...
df = pd.read_pickle(
    f'/home/bbb1417/academatepy/validation/PICOS/preprocessed_articles_filtered.pkl')
output_dir = f"{workdir}/{test_result_type}"

print(df.columns)

df.screening1.value_counts()
df1 = df[df.screening1==True]
with open(f"{workdir}/id2pdf.pkl", 'rb') as file:  # Open in binary read mode ('rb')
    id2pdf = pickle.load(file)
    print(f"id2pdf Dictionary successfully loaded")
