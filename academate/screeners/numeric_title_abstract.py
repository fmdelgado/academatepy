# academate/screeners/numeric_title_abstract.py
from langchain.prompts import ChatPromptTemplate
import pandas as pd
from academate.screeners.title_abstract import TitleAbstractScreener
import json

class NumericTitleAbstractScreener(TitleAbstractScreener):
    """Screener for titles and abstracts with numeric ratings."""

    def __init__(self, llm, criteria_dict, vector_store_path, verbose=False, selection_method="threshold"):
        """
        Initialize the numeric title/abstract screener.
        ...
        Args:
            ...
            selection_method (str, optional): Method for article selection.
                                             Options: 'threshold', 'clustering'.
                                             Defaults to 'threshold'.
        """
        super().__init__(llm, criteria_dict, vector_store_path, "screening1", verbose)
        self.selection_method = selection_method
        self.threshold = 0.7  # Default threshold value

    def prepare_prompt(self):
        """
        Prepare a prompt template that asks for numeric ratings instead of boolean values.
    ...
    def _prepare_results(self, data):
        ...
        if not results_df.empty:
            ...
            if self.selection_method == "threshold":
                ...
                results_df['predicted_screening1'] = results_df[criteria_columns].mean(axis=1) >= self.threshold
